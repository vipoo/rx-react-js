...
exports.memoriseLastValue = memoriseLastValue;
...
function immediateThrottlePromise(fnPromise, period) {
  ...
  var keyFn = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : function () {
    return false;
  };
  ...
  return _Observable.Observable.create(function (observer) {
    ...
    function delayEmit(emitKey) {
      ...
      timer = setTimeout(function () {
        if (requestAgainAfterPeriod) emit(emitKey);
    ...
    function emit(emitKey) {
      ...
      fnPromise(captured).then(function (data) {
        return subscribed && emitKey == lastKey ? observer.next(data) : null;
      }).catch(function (err) {
        clearTimeout(timer);
        timer = null;
        if (subscribed) observer.error(err);
      }).tap(function () {
        return emitKey == lastKey ? delayEmit(emitKey) : null;
      }).finally(function () {
        if (emitKey == lastKey) waitingForPromise = false;
      });
    }
...
function memoriseLastValue() {
  var _context2;

  var autoSubscription = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;

  return (_context2 = shared.call(this), replayLastValue).call(_context2, autoSubscription);
}